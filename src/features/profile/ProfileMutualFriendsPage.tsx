...
export function ProfileMutualFriendsPage({ username }: { username: string }) {
  ...
  const load = useCallback(async (cur: string | null, append: boolean) => {
    ...
    setError(null);
    try {
      const r = await profileApi.getMutualFriendsList(username, { cursor: cur ?? undefined, limit: 20 });
      if (mySeq !== seq.current) return;
      setItems(prev => append ? [...prev, ...r.items] : r.items);
      setCursor(r.nextCursor);
    } catch (e) {
      if (mySeq !== seq.current) return;
      setError(e instanceof ApiError ? e.message : 'Failed to load mutual friends.');
    } finally {
  ...
  return (
    <div className="page">
      <div style={{ marginBottom: 16 }}>
        ...
        <h1 className="font-display" style={{ fontSize: 20, marginTop: 6 }}>Mutual friends with @{username}</h1>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 24 }}>
          ...
          <Button size="sm" variant="ghost" onClick={() => load(null, false)}>Retry</Button>
        </div>
      ) : !items.length ? (
        <EmptyState
          icon="users"
          title="No mutual friends"
          body="You and this player have no mutual friends visible to you."
        />
      ) : (
        <div className="col" style={{ gap: 2 }}>
          ...
          {cursor && (
            <div style={{ textAlign: 'center', marginTop: 12 }}>
              <Button size="sm" variant="ghost" disabled={loadingMore} onClick={() => load(cursor, true)}>
